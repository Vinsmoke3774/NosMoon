...
namespace OpenNos.GameObject
{
    public class MapInstance : BroadcastableBase
    {
        ...
        public byte InstantBattleWaves { get; set; }
        ...
        public Mate GetMate(long mateTransportId) => Sessions.SelectMany(s => s.Character?.Mates.ToList()).FirstOrDefault(m => m.MateTransportId == mateTransportId);
        ...
        {
            Logger.Log.LogUserEvent("PUTITEM", session.GenerateIdentity(), $"type: {type} slot: {slot} amount: {amount}");
            Guid random2 = Guid.NewGuid();
            MapItem droppedItem = null;
            List<Tile> possibilities = new();

            for (short x = -2; x < 3; x++)
            {
                for (short y = -2; y < 3; y++)
                {
                    possibilities.Add(new Tile { X = x, Y = y });
                }
            }
        ...
        public void RemoveMapItem()
        {
            ...
            try
            {
                IEnumerable<MapItem> dropsToRemove = DroppedList.Values.Where(dl => dl.CreatedDate.AddMinutes(3) < DateTime.Now);
                Parallel.ForEach(dropsToRemove, drop =>
                {
                    Broadcast(StaticPacketHelper.Out(UserType.Object, drop.TransportId));
                    DroppedList.TryRemove(drop.TransportId, out _);
                });
            }
            catch (Exception e)
            {
        ...
        public void ThrowItems(Tuple<int, short, byte, int, int, byte, byte, short> parameter)
        {
            ...
            short destY;
            int amount = 1;
            Observable.Timer(TimeSpan.FromSeconds(parameter.Item6)).SafeSubscribe(s =>
            {
                for (int i = 0; i < parameter.Item3; i++)
                {
                    ...
                    MonsterMapItem droppedItem = new(destX, destY, parameter.Item2, amount, minRarity: parameter.Item6, maxRarity: parameter.Item7);
                    if (parameter.Item6 != 0 && parameter.Item7 != 0)
                    {
                        droppedItem.RarifyV2();
                    }
                    DroppedList[droppedItem.TransportId] = droppedItem;
        ...
        internal void CreatePortal(Portal portal, int timeInSeconds = 0, bool isTemporary = false)
        {
            ...
            if (isTemporary)
            {
                Observable.Timer(TimeSpan.FromSeconds(timeInSeconds)).SafeSubscribe(o =>
                {
                    if (portal.SourceMapInstanceId == null)
                    {
                        return;
                    }
                    if (portal != null)
        ...
        {
            _mapLifeDisposable = Observable.Interval(TimeSpan.FromSeconds(1)).SafeSubscribe(x =>
            {
        ...
        internal int SummonMonster(MonsterToSummon summon, bool generate = true)
        {
            ...
            Observable.Timer(TimeSpan.FromSeconds(summon.HasDelay))
                .SafeSubscribe(o =>
                {
                    ...
                    {
                        _generateDelay.Add(mapMonster.GenerateIn());
                    }
        ...
        private readonly ConcurrentBag<string> _generateDelay = new ConcurrentBag<string>();

        internal void SummonMonsters(List<MonsterToSummon> monstersToSummon, bool generate = true)
        {
            foreach (var monsterToSummon in monstersToSummon)
            {
                SummonMonster(monsterToSummon, generate);
            }
            ...
            {
                foreach (var player in Sessions.Where(s => s.HasCurrentMapInstance))
                {
                    player.SendPackets(_generateDelay, cond: false);
                    await Task.Delay(100);
                }

                _generateDelay.Clear();
            });
        }
                            ...
                            Parallel.ForEach(GetBattleEntitiesInRange(new MapCell { X = mapMonster.MapX, Y = mapMonster.MapY }, ski.Skill.Range),
                                x =>
                                {
                                    ...
                                    if (x.MapMonster?.Owner is BattleEntity owner)
                                    {
                                        ...
                                        bool zephyrhWings = false;

                                        damage = DamageHelper.Instance.CalculateDamage(owner, x, ski.Skill, ref hitMode, ref onyxWings, ref zephyrhWings);
                                    }
                                    else
                                    {
                                    ...
                                    if (x.Hp < 1)
                                    {
                                        ...
                                        if (x.Character != null)
                                        {
                                            Observable.Timer(TimeSpan.FromSeconds(1)).SafeSubscribe(c => ServerManager.Instance.AskRevive(x.Character.CharacterId));
                                        }
                                    }
                                });
                        });
