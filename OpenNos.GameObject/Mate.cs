    ...
    public class Mate : MateDTO
    {
        ...
        public Mate(MateDTO input)
        {
            ...
            if (Monster.CriticalChance == 0 && Monster.CriticalRate == 0)
            {
                ...
                {
                    Logger.Log.Warn("MissingMateStats.txt was in use, but i was able to catch this exception");
                }
            }
        }
        ...
        public Mate(Character owner, NpcMonster npcMonster, byte level, MateType matetype, bool temporal = false, bool tsReward = false, bool tsProtected = false)
        {
            ...
            Monster = npcMonster;
            Level = level;
            Hp = MateHelper.Instance.EggTypeMates.Any(s => s.Equals(NpcMonsterVNum)) ? 1 : MaxHp;
            Mp = MaxMp;
        ...
        public int CriticalChance => CriticalChanceLoad();

        public int CriticalRate => CriticalRateLoad();
        ...
        public bool IsEgg => BattleEntity == null ? MateHelper.Instance.EggTypeMates.Any(s => s.Equals(NpcMonsterVNum)) : BattleEntity.IsEggPet;
        ...
        private int EquipmentDarkResistance => GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.DarkIncreased)[0] + GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.AllIncreased)[0];

        private int EquipmentFireResistance => GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.FireIncreased)[0] + GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.AllIncreased)[0];

        private int EquipmentLightResistance => GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.LightIncreased)[0] + GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.AllIncreased)[0];

        private int EquipmentWaterResistance => GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.WaterIncreased)[0] + GetStuffBuff(CardType.ElementResistance, (byte)AdditionalTypes.ElementResistance.AllIncreased)[0];

        #endregion

        #region Methods

        public int CriticalChanceLoad()
        {
            if (IsTemporalMate)
            {
                return Monster.CriticalChance;
            }

            return MateHelper.Instance.GetCriticalChanceData(Monster, MateType == MateType.Partner);
        }

        public int CriticalRateLoad()
        {
            if (IsTemporalMate)
            {
                return Monster.CriticalChance;
            ...
            return MateHelper.Instance.GetCriticalData(Monster, MateType == MateType.Partner);
        }
        ...
        public void StartLife()
        {
            if (IsTeamMember)
            {
                Life = Observable.Interval(TimeSpan.FromMilliseconds(500)).SafeSubscribe(x => { MateLife(); });
            }
        }
        ...
        public void MateLife()
        {
            if (IsEgg)
            {
                ...
                if (Hp >= MaxHp && Monster.EvolutionVNum.HasValue)
                {
                    Owner.ReplacePet(this, Monster.EvolutionVNum.Value, 1);
                }
            }
        }
        ...
        public void AddTeamMember()
        {
            ...
            {
                if (Owner?.Session?.CurrentMapInstance?.MapInstanceType == MapInstanceType.ArenaInstance)
                {
                    return;
                }
            }
            */
            ...
            Observable.Timer(TimeSpan.FromSeconds(2)).SafeSubscribe(s =>
            {
                if (IsTeamMember)
                {
                    if (Owner != null && Monster.BuffId.HasValue)
                    {
                        Owner.AddBuff(new Buff(Monster.BuffId.Value, Level, isPermaBuff: true), BattleEntity);
                    }
                }
            });
        ...
        public int ConcentrateLoad()
        {
            ...
            return MateHelper.Instance.LoadConcentrate(Monster, Level, MateType == MateType.Partner);
        }
        ...
        public int DamageMaximumLoad()
        {
            ...
            return MateHelper.Instance.LoadMaxMinDamageData(Monster, Level, MateType == MateType.Partner, false);
        }
        ...
        public int DamageMinimumLoad()
        {
            ...
            return MateHelper.Instance.LoadMaxMinDamageData(Monster, Level, MateType == MateType.Partner, true);
        }
        ...
        public void GenerateDeath(BattleEntity killer)
        {
            ...
            if (MateType == MateType.Pet ? Owner.IsPetAutoRelive : Owner.IsPartnerAutoRelive)
            {
                ...
                else
                {
                    ...
                    ReviveDisposable = Observable.Timer(TimeSpan.FromMinutes(3)).SafeSubscribe(s => GenerateRevive());
                    Owner.Session.SendPacket(UserInterfaceHelper.GenerateMsg(
        ...
        public string GenerateIn(bool hideNickname = false, bool isAct4 = false, AuthorityType receiverAuthority = AuthorityType.User)
        {
            ...
            var fuckingCrapShit = Sp?.GenerateSkills(false);
            var effect1 = fuckingCrapShit?.Effect1 ?? 0;
            var effect2 = fuckingCrapShit?.Effect2 ?? 0;
            var effect3 = fuckingCrapShit?.Effect3 ?? 0;
            return $"in 2 {NpcMonsterVNum} {MateTransportId} {PositionX} {PositionY} {Direction} {(int)(Hp / MaxHp * 100)} {(int)(Mp / MaxMp * 100)} 0 {faction} 3 {CharacterId} 1 0 {(IsUsingSp && Sp != null ? Sp.Instance.Item.Morph : (Skin != 0 ? Skin : -1))} {name} {(Sp != null ? 1 : 0)} {(IsUsingSp ? 1 : 0)} {(IsUsingSp ? 1 : 0)}{(IsUsingSp ? fuckingCrapShit?.ApparentlySkillVnumsBecauseSaluCodesLikeCrapShit : " 0 0 0")} {(IsUsingSp ? effect1 : 0)} {(IsUsingSp ? effect2 : 0)} {(IsUsingSp ? effect3 : 0)} 0";
        }
        ...
        public void GenerateRevive()
        {
            ...
            Owner.MapInstance?.Broadcast(GenerateIn(false, ServerManager.Instance.ChannelId == 51));
            if (Loyalty <= 100)
            {
            ...
            switch (MateType)
            {
                case MateType.Partner:
                    return
                        ...
                        $"{(Sp != null ? Sp.GenerateSkills().ApparentlySkillVnumsBecauseSaluCodesLikeCrapShit : " -1 -1 -1")}";
        ...
        public void GenerateXp(int xp)
        {
            ...
            if (Level < ServerManager.Instance.Configuration.MaxLevel)
            {
                ...
                Experience += (int)(xp * (1 + (Owner.Buff.ContainsKey(122) ? 0.5 : 0) + medalBonus));
        ...
        {
            return MateType == MateType.Pet ? new List<ItemInstance>() : Owner.Inventory.Values.Where(s => s.Type == (InventoryType)(13 + PetId)).ToList();
        }
        ...
        public int[] GetStuffBuff(CardType type, byte subtype)
        {
            ...
            int value1 = 0;
            int value2 = 0;
            foreach (BCard entry in EquipmentBCards.Where(s => s.Type.Equals((byte)type) && s.SubType.Equals((byte)(subtype)) && s.FirstData > 0))
            {
        ...
        public void HitRequest(HitRequest hitRequest)
        {
            if (IsAlive && (hitRequest.Session?.Character == null || hitRequest.Session.Character.Hp > 0) && (hitRequest.Mate == null || hitRequest.Mate.Hp > 0))
            {
                ...
                {
                    cooldownReduction = hitRequest.Session.Character.GetBuff(CardType.Morale, (byte)AdditionalTypes.Morale.SkillCooldownDecreased)[0] * -1;
                ...
                if (attackerBattleEntity.Character != null)
                {
                    ...
                    if (Hp <= 0)
                    {
                        if (hitRequest.SkillBCards.FirstOrDefault(s => s.Type == (byte)CardType.TauntSkill && s.SubType == (byte)AdditionalTypes.TauntSkill.EffectOnKill) is BCard EffectOnKill)
                        {
        ...
        public int HpLoad()
        {
            ...
            return (int)((MateHelper.Instance.LoadHpData(Monster, Level) + hp) * multiplicator);
        }
        ...
        public int MagicalDefenseLoad()
        {
            ...
            return MateHelper.Instance.LoadDefences(Monster, Level, MateType == MateType.Partner, 2);
        }
        ...
        public int MeleeDefenseDodgeLoad()
        {
            ...
            return MateHelper.Instance.LoadDefences(Monster, Level, MateType == MateType.Partner, 0);
        }
        ...
        public int MeleeDefenseLoad()
        {
            ...
            return MateHelper.Instance.LoadDefences(Monster, Level, MateType == MateType.Partner, 1);
        }
        ...
        public int MpLoad()
        {
            ...
            return (int)((MateHelper.Instance.LoadMpData(Monster, Level) + mp) * multiplicator);
        }
        ...
        public void RemoveSp(bool isBackToMiniland = false)
        {
            ...
            if (IsTeamMember)
            {
                ...
                Owner.Session.SendPacket(GenerateCond());
                Owner.MapInstance.Broadcast(GenerateOut());
                MateHelper.Instance.RemoveMateBuffs(this);
        ...
        public void StartSpCooldown()
        {
            if (Sp != null)
            {
                ...
                Observable.Timer(TimeSpan.FromSeconds(SpCooldown)).SafeSubscribe(o => Owner?.Session?.SendPacket("psd 0"));
        ...
        public void TargetHit2(BattleEntity target, NpcMonsterSkill npcMonsterSkill, int damage, int hitmode)
        {
                        ...
                        if (target.Hp <= 0)
                        {
                            if (target.Character != null)
                            {
                                ...
                                if (Owner.MapInstance?.MapInstanceType != MapInstanceType.BattleRoyaleInstance)
                                {
                                    ...
                                    Owner.MapInstance?.Broadcast((UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("PVP_KILL"),
                                        Owner.Name, target.Character.Name), 0)));
                                }
            ...
            if (npcMonsterSkill != null && (npcMonsterSkill.Skill.Range > 0 || npcMonsterSkill.Skill.TargetRange > 0))
            {
                bool onyxWings = false;
                bool zephyrWings = false;
                var mapInstance = BattleEntity?.MapInstance;

                if (mapInstance == null)
                {
                    return;
                }
                foreach (Character characterInRange in mapInstance.GetCharactersInRange(npcMonsterSkill.Skill?.TargetRange == 0 ? MapX : RangeBaseX, npcMonsterSkill.Skill?.TargetRange == 0 ? MapY : RangeBaseY,
                        npcMonsterSkill.Skill.TargetRange).Where(s => s.CharacterId != target?.MapEntityId))
                {
                    if (characterInRange != null && !BattleEntity.CanAttackEntity(characterInRange?.BattleEntity))
                    {
                        ...
                        if (characterInRange.Hp > 0)
                        {
                            ...
                            if (characterInRange.Hp <= 0)
                            {
                                ...
                                if (Owner.MapInstance?.MapInstanceType != MapInstanceType.BattleRoyaleInstance)
                                {
                                    ...
                                    Owner.MapInstance?.Broadcast((UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("PVP_KILL"),
                                        Owner?.Name, characterInRange?.Name), 0)));
                                }
