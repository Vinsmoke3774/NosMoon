    ...
    public class CharacterSkill : CharacterSkillDTO
    {
        ...
        public List<BCard> GetSkillBCards()
        {
            ...
            if (ServerManager.Instance.GetSessionByCharacterId(CharacterId) is ClientSession Session)
            {
                ...
                {
                    foreach (BCard ambushBCard in Session.Character.Buff.GetAllItems().SelectMany(s => s.Card.BCards.Where(b => b.Type == (byte)CardType.FearSkill && b.SubType == (byte)AdditionalTypes.FearSkill.ProduceWhenAmbushe)))
                    {
                ...
                {
                    foreach (BCard sniperAttackBCard in Session.Character.Buff.GetAllItems().SelectMany(s => s.Card.BCards.Where(b => b.Type == (byte)CardType.SniperAttack && b.SubType == (byte)AdditionalTypes.SniperAttack.ChanceCausing)))
                    {
                        SkillBCards.Add(sniperAttackBCard);
                    }
                }
                foreach (BCard ambushAttackBCard in Session.Character.Buff.GetAllItems().SelectMany(s => s.Card.BCards.Where(b => b.Type == (byte)CardType.SniperAttack && b.SubType == (byte)AdditionalTypes.SniperAttack.ProduceChance)))
                {
