        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte Option = 0, string[] packetsplit = null)
        {
            ...
            switch (Effect)
            {
                default:
                    ...
                    if (item.BCards.Find(s => s.Type == 25) is BCard Buff && item.VNum != 2028 /* Temp disable for battle pots */)
                    {
            ...
            if (session.Character.SnackRequests > 60)
            {
                PenaltyLogDTO log = new PenaltyLogDTO
                {
                    AccountId = session.Character.AccountId,
                    Reason = "Auto Ban Snack Infinite Abuse PL",
