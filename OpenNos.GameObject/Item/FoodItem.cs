...
namespace OpenNos.GameObject
{
    public class FoodItem : Item
    {
        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte Option = 0, string[] packetsplit = null)
        {
            ...
            if (ServerManager.Instance.ChannelId == 51 || ServerManager.Instance.ChannelId == 4)
            {
                return;
            }
            ...
            switch (Effect)
            {
                default:
                    ...
                    if (amount < 5)
                    {
                        if (item.BCards.Find(s => s.Type == (byte)BCardType.CardType.HPMP && s.SubType == (byte)AdditionalTypes.HPMP.ReceiveAdditionalHP && s.FirstData > 0) is BCard AdditionalHpBCard)
                        {
                            ...
                            if (AdditionalHp > 0 && AdditionalHp <= AdditionalHpBCard.FirstData)
                            {
                                ...
                                session.Character.Inventory.RemoveItemFromInventory(inv.Id);
                                Observable.Timer(TimeSpan.FromMilliseconds(1800)).SafeSubscribe(s =>
                                {
                        ...
                        if (item.BCards.Find(s => s.Type == (byte)BCardType.CardType.HPMP && s.SubType == (byte)AdditionalTypes.HPMP.ReceiveAdditionalMP && s.FirstData < 0) is BCard AdditionalMpBCard)
                        {
        ...
        private static void Regenerate(ClientSession session, Item item)
        {
            ...
            _regenerateDisposable = Observable.Timer(TimeSpan.FromMilliseconds(1800 * 5)).SafeSubscribe(obs =>
            {
                if (session?.Character == null)
                {
