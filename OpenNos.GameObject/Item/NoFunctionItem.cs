                ...
                case 10000:
                    {
                        ...
                        {
                            case 1:
                                if (session.Character.Inventory.CountItem(20042) < 1 || session.Character.Inventory.CountItem(20043) < 1)
                                {
                                    return;
                                }
                                session.Character.Inventory.RemoveItemAmount(20042);
                                session.Character.Inventory.RemoveItemAmount(20043);
                                break;
                        }
                        session.Character.GiftAdd(20036, (byte)ServerManager.RandomNumber(5, 11));
                    }
                    break;
                ...
                case 0:
                    switch (VNum)
                    {
                        case 1086:
                            ...
                            {
                                session?.SendPacket(UserInterfaceHelper.GenerateMsg("You can't receive Sound Flower Quest in Act4", 0));
                                break;
                            }

                            if (session.Character.AddQuest(ServerManager.Instance.FlowerQuestId.Value))
                            {
