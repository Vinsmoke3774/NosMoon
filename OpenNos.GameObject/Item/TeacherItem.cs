    ...
    public class TeacherItem : Item
    {
        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte Option = 0, string[] packetsplit = null)
        {
            ...
            switch (Effect)
            {
                ...
                case 13:
                    if (int.TryParse(packetsplit[3], out mateTransportId) && session.Character.Mates.ToList().FirstOrDefault(s => s.MateTransportId == mateTransportId) is Mate pet)
                    {
                ...
                case 0:
                    if (VNum == 5262)
                    {
                        ...
                        var mate = session.Character.Mates.GetAllItems().FirstOrDefault(s => s.MateTransportId == mateTransportId);
                        ...
                        var increase = (int)(mate.MaxHp / 4);
                        mate.Hp += increase;

                        if (mate.Hp > mate.MaxHp)
                        {
                            mate.Hp = mate.MaxHp;
                        }
                        session.Character.Inventory.RemoveItemFromInventory(inv.Id);
                    ...
                    else if (VNum == 2404)
                    {
                        ...
                        var random = ServerManager.RandomNumber<short>();

                        if (!mate.Monster.EvolutionVNum.HasValue || mate.IsEgg)
                        {
                            session.SendPacket(UserInterfaceHelper.GenerateInfo("This pet cannot evolve !"));
                            return;
                        }

                        session.Character.Inventory.RemoveItemFromInventory(inv.Id);

                        var evolutionChance = mate.Monster.EvolutionChance <= 0 ? 100 : mate.Monster.EvolutionChance;

                        if (random >= evolutionChance)
                        {
                            session.SendPacket(UserInterfaceHelper.GenerateInfo("The pet transformation failed!"));
                            return;
                        }

                        session.Character.ReplacePet(mate, mate.Monster.EvolutionVNum.Value, mate.Level);
                    }
                    else
                    {
