        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte Option = 0, string[] packetsplit = null)
        {
            switch (Effect)
            {
                default:
                    ...
                    if (!((int)inv.Item.Effect).IsBetween(790, 796) && ItemValidTime > 0 && !inv.IsBound)
                    {
                    ...
                    if (!inv.IsBound)
                    {
                        ...
                        {
                            if (Sex != 0 && Sex != (byte)session.Character.Gender + 1 || ((Class >> (byte)session.Character.Class) & 1) != 1)
                            {
                                session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("BAD_EQUIPMENT"), 0));
                                return;
                            }
                            ...
                            case ItemType.Weapon:
                                switch (inv.Item.EquipmentSlot)
                                {
                                    case EquipmentType.MainWeapon:
                                        ...
                                        foreach (var dto in inv.RuneEffects.Where(s => s.IsRune))
                                        {
