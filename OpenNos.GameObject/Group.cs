...
namespace OpenNos.GameObject
{
    public class Group : IDisposable
    {
        ...
        public List<string> GeneratePst(ClientSession player)
        {
            ...
            Sessions.Where(s => s == player).ForEach(session =>
            {
                i = session.Character.Mates.ToList().Count(s => s.IsTeamMember);
                str.Add($"pst 1 {session.Character.CharacterId} {++i} {(int)(session.Character.Hp / session.Character.HPLoad() * 100)} {(int)(session.Character.Mp / session.Character.MPLoad() * 100)} {session.Character.HPLoad()} {session.Character.MPLoad()} {(byte)session.Character.Class} {(byte)session.Character.Gender} {(session.Character.UseSp ? session.Character.Morph : 0)}");
            });
            return str;
        }
        ...
        public string GenerateRdlst()
        {
            ...
            if (GroupType == GroupType.Team || GroupType == GroupType.BigTeam)
            {
            ...
            {
                Logger.Log.Error(nameof(GenerateRdlst), ex);
            }
        ...
        public void JoinGroup(long characterId, bool bypassBlacklist)
        {
            ClientSession session = ServerManager.Instance.GetSessionByCharacterId(characterId);

            if (GroupType >= GroupType.BigTeam && GroupType <= GroupType.GiantTeam &&
                session?.CurrentMapInstance?.MapInstanceType == MapInstanceType.ArenaInstance)
            {
                return;
            }
            ...
            if (session != null)
            {
                JoinGroup(session, bypassBlacklist);
            }
        }

        public bool JoinGroup(ClientSession session, bool bypassBlacklist)
        {
            ...
            if (GroupType == GroupType.BigTeam || GroupType == GroupType.GiantTeam || GroupType == GroupType.Team || GroupType == GroupType.LargeTeam)
            {
                var maxGroupSize = (int) GroupType;

                if (Sessions.Count >= maxGroupSize)
                {
                    session.SendPacket(session.Character.GenerateSay($"The raid team is already full.", 10));
                    return false;
                }

                if (Sessions.CountLinq(se => se.CleanIpAddress.Equals(session.CleanIpAddress)) > 2)
                {
