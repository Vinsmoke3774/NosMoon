...
namespace OpenNos.GameObject.Networking
{
    public class ServerManager : BroadcastableBase
    {
        ...
        public List<ClientSession> TimespaceSessions = new();
        ...
        private ConcurrentBag<BCardDTO> _bcards;
        ...
        {
            _bcards = new ConcurrentBag<BCardDTO>();
        }
        ...
        public short KingOfTheHillTimer { get; set; } = 20;
        ...
        public ConcurrentBag<ChatLogDTO> ChatLogs { get; set; }
        ...
        public ThreadSafeGenericLockedList<KingOfTheHillMember> KingOfTheHillMembers { get; set; } = new();

        public ThreadSafeGenericLockedList<ArenaMember> ArenaMembers { get; set; } = new();

        public ThreadSafeGenericLockedList<DeathMatchMember> DeathMatchMembers { get; set; } = new();

        public ThreadSafeGenericLockedList<OneVersusOneMember> OneVersusOneMembers { get; set; } = new();

        public ThreadSafeGenericLockedList<TwoVersusTwoMember> TwoVersusTwoMembers { get; set; } = new();

        public List<ConcurrentBag<ArenaTeamMember>> ArenaTeams { get; set; } = new List<ConcurrentBag<ArenaTeamMember>>();

        public List<long> BannedAccounts { get; set; } = new List<long>();
        ...
        public List<TwoVersusTwoBattleTeam> TwoVersusTwoBattleMembers { get; set; } = new List<TwoVersusTwoBattleTeam>();
        ...
        public ThreadSafeGenericList<EventType> StartedEvents { get; set; }
        ...
        public ConcurrentBag<ScriptedInstance> SkyTowers { get; set; }

        public List<CharacterTimespaceLogDTO> TimespaceLogs = new();

        public ConcurrentBag<TitleWearConditionDTO> TitleWearConditions { get; set; }

        public ConcurrentBag<LevelUpRewardDTO> LevelUpRewards { get; set; }

        public FactionType LastFcRaid { get; set; }

        public List<PrivateMapArena> PrivateArenaMaps = new();

        public List<BattlePassItemDTO> BattlePassItems = new();

        public List<BattlePassPalierDTO> BattlePassPaliers = new();

        public List<BattlePassQuestDTO> BattlePassQuests = new();

        public byte _currentMessagesIndex;

        public List<string> MessagesLoop = new()
        {
            "Please report any bugs you find through tickets.",
            "Do not share your account password with anyone !",
            "The main language of the server is English, please use English in general chats."
        };
        ...
        public static MapInstance ResetMapInstance(MapInstance baseMapInstance)
        {
            if (baseMapInstance != null)
            {
                ...
                foreach (ScriptedInstanceDTO si in DAOFactory.ScriptedInstanceDAO.LoadByMap(mapInstance.Map.MapId).ToList())
                {
                    ...
                    if (siObj.Type == ScriptedInstanceType.SkyTower)
                    {
                        mapInstance.ScriptedInstances.Add(siObj);
                    }
        ...
        public void RemoveGroup(Group group) => ThreadSafeGroupList.Remove(group.GroupId);
        ...
        public void AskPvpRevive(long characterId)
        {
            ...
            switch (session.CurrentMapInstance.MapInstanceType)
            {
                case MapInstanceType.TwoVersusTwo:
                    ...
                    session.SendPacket(session.Character.GenerateCond());
                    session.Character.TwoVersusTwoBattle.IsDead = true;
                    break;

                case MapInstanceType.RainbowBattleInstance:
                    if (DateTime.Now < session.Character.LastRbbDeath.AddSeconds(2))
                    {
                        return;
                    }

                    session.Character.LastRbbDeath = DateTime.Now;
                    var rbb = RainbowBattleMembers.Find(s => s.Session.Contains(session));
                    ...
                    Observable.Timer(TimeSpan.FromSeconds(20)).SafeSubscribe(o =>
                    {
                        ...
                        if (session.Character.IsFrozen)
                        {
                            session.Character.PositionX = rbb.TeamEntity == RainbowTeamBattleType.Red ? RandomNumber<short>(30, 32) : RandomNumber<short>(83, 85);
                            session.Character.PositionY = rbb.TeamEntity == RainbowTeamBattleType.Red ? RandomNumber<short>(73, 76) : RandomNumber<short>(2, 4);
        ...
        public void AskRevive(long characterId)
        {
            ...
            switch (session.CurrentMapInstance?.MapInstanceType)
            {
                    ...
                    else
                    {
                        if (session.Character != null)
                        {
                            Observable.Timer(TimeSpan.FromSeconds(10)).SafeSubscribe(o =>
                            {
                                ...
                                ReviveFirstPosition(session.Character.CharacterId);
                            });
                        }
                    }
                    break;
                ...
                case null: // Seems to happen somehow ?
                    session.Disconnect();
                    break;
        ...
        // Both partly
        public void ChangeMapInstance(long characterId, Guid mapInstanceId, int? mapX = null, int? mapY = null, bool noAggroLoss = false, int? dir = 0)
        {
            ClientSession session = GetSessionByCharacterId(characterId);
            ...
            session.Character.OldMapId = session.Character.MapId;
            ...
            try
            {
                ...
                if (gotoMapInstance.Map.MapId == 20001 && ChannelId == 51)
                {
                ...
                if (gotoMapInstance == CaligorRaid.CaligorMapInstance && session.Character.MapInstance != CaligorRaid.CaligorMapInstance)
                {
                    ...
                    if (CaligorRaid.CaligorMapInstance.Sessions.Count(s => s.Character?.Faction == FactionType.Angel) >
                        CaligorRaid.CaligorMapInstance.Sessions.Count(s => s.Character?.Faction == FactionType.Demon)
                        && session.Character.Faction != FactionType.Demon)
                    {
                        session.Character.Faction = FactionType.Demon;
                        session.SendPacket(session.Character.GenerateFaction());
                    }
                    else if (CaligorRaid.CaligorMapInstance.Sessions.Count(s => s.Character?.Faction == FactionType.Demon) >
                             CaligorRaid.CaligorMapInstance.Sessions.Count(s => s.Character?.Faction == FactionType.Angel)
                ...
                if (!session.Character.MapInstance.MapInstanceType.Equals(MapInstanceType.TimeSpaceInstance) && session.Character.Timespace != null)
                {
                    ...
                    if (session.Character.Timespace.SpNeeded?[(byte)session.Character.Class] != 0)
                    {
                        ...
                        {
                            Observable.Timer(TimeSpan.FromMilliseconds(300)).SafeSubscribe(s => session.Character.RemoveSp(specialist.ItemVNum, true));
                        }
                    }
                ...
                if (session.Character.MapInstance.MapInstanceType == MapInstanceType.BaseMapInstance)
                {
                    ...
                    if (mapX != null && mapY != null)
                    {
                        ...
                        session.Character.Direction = (byte)dir.Value;
                    }
                }
                ...
                session.SendPacket(session.Character.GenerateCond());
                session.SendPacket(session.Character.GenerateCMap(true));
                session.SendPackets(session.Character.GenerateStatChar());
                session.SendPacket(session.Character.GeneratePairy());
                session.SendPacket(session.Character.GenerateScpStc());
                session.CurrentMapInstance.Broadcast(session.Character.GenerateTitInfo());
                session.Character.SendAscrPacket();
                session.SendPacket(session.Character.GenerateDir());

                if (session.CurrentMapInstance?.MapInstanceType != MapInstanceType.SkyTowerInstance)
                {
                    session.SendPacket(session.Character.GenerateMinimapPosition());
                }

                if (session.CurrentMapInstance.OnSpawnEvents.Count > 0)
                {
                    session.CurrentMapInstance.OnSpawnEvents.ForEach(e => EventHelper.Instance.RunEvent(e, session: session));
                }

                session.SendPacket(session.Character.MapInstance.Map.MapId >= 228 && session.Character.MapInstance.Map.MapId <= 238 || session.Character.MapInstance.Map.MapId == 2604 ? session.Character.GenerateAct6() : session.Character.GenerateAct());
                ...
                foreach (var visibleSession in session.CurrentMapInstance.Sessions.Where(s => s.Character?.InvisibleGm == false && s.Character.CharacterId != session.Character.CharacterId))
                {
                    if (ChannelId != 51 || session.Character.Faction == visibleSession.Character.Faction)
                    {
                        ...
                        visibleSession.Character.Mates.Where(m => (m.IsTeamMember || m.IsTemporalMate) && m.CharacterId != session.Character.CharacterId).ToList().ForEach(m => session.SendPacket(m.GenerateIn()));
                    }
                    else
                    {
                        session.SendPacket(visibleSession.Character.GenerateIn(true, session.Account.Authority));
                        visibleSession.Character.Mates.Where(m => (m.IsTeamMember || m.IsTemporalMate) && m.CharacterId != session.Character.CharacterId).ToList().ForEach(m => session.SendPacket(m.GenerateIn(true, ChannelId == 51, session.Account.Authority)));
                    }
                };
                ...
                // Wp portal
                session.CurrentMapInstance.ScriptedInstances.ToList().ForEach(ts =>
                {
                    var getTs = TimespaceLogs.Find(s => s.ScriptedInstanceId == ts.ScriptedInstanceId && s.CharacterId == session.Character.CharacterId && !s.IsFailed);
                    bool isTsFound = getTs != null;
                    var isHeroTimeSpace = ts.DefaultTimeSpaceType == WpPortalType.HeroTs;
                    session.SendPacket(ts.GenerateWp(isHeroTimeSpace && isTsFound ? WpPortalType.HeroTsDone : !isHeroTimeSpace && isTsFound ? WpPortalType.NormalTsDone : ts.DefaultTimeSpaceType));
                });
                ...
                MapInstancePortalHandler.GenerateAct4EntryPortals(session.CurrentMapInstance.Map.MapId, session.Character.Faction).ForEach(p => session.SendPacket(p.GenerateGp()));
                ...
                if (!session.Character.InvisibleGm)
                {
                    foreach (var s in session.CurrentMapInstance.Sessions.Where(s => s.Character != null))
                    {
                        ...
                        session.Character.Mates.Where(m => m.IsTeamMember || m.IsTemporalMate).ToList().ForEach(m =>
                        {
                            if (session.Character.IsVehicled)
                            {
                ...
                if (session.Character.Mates.ToList().FirstOrDefault(s => (s.IsTeamMember || s.IsTemporalMate) && s.MateType == MateType.Partner && s.IsUsingSp) is Mate partner)
                {
                ...
                {
                    session.SendPacket(session.Character.GenerateAt());
                    session.SendPacket("rsfm 801 0 7 6 1");
                    session.SendPacket("guri 41 1");
                    session.SendPacket("msgi 0 2470 0 0 0 0 0");
                    session.SendPacket("ctl 2 1 0 99");
                }
            ...
            {
                Logger.Log.Error("Character changed while changing map. Do not abuse Commands.", ex);
                session.Character.IsChangingMapInstance = false;
            }
        }
        ...
        public void GroupLeave(ClientSession session)
        {
            ...
            switch (grp.GroupType)
            {
                ...
                case GroupType.Group:
                    ...
                    if (session.Character.MapInstance.MapInstanceType == MapInstanceType.TwoVersusTwo)
                    {
                        return;
                    }
        ...
        {
            OrderablePartitioner<ItemDTO> items = Partitioner.Create(DAOFactory.ItemDAO.LoadAll(), EnumerablePartitionerOptions.NoBuffering);

            Parallel.ForEach(items, new ParallelOptions { MaxDegreeOfParallelism = 8, TaskScheduler = TaskScheduler.Current }, itemDto =>
            {
            ...
            Logger.Log.Info($"{_items.Count} items loaded.");
        }
        ...
        private void LoadMonsterDrops()
        {
            ...
            var monsters = DAOFactory.DropDAO.LoadAll().GroupBy(d => d.MonsterVNum);
            foreach (var monsterDropGrouping in monsters)
            {
        ...
        private void LoadMonsterSkills()
        {
            ...
            var skills = DAOFactory.NpcMonsterSkillDAO.LoadAll().GroupBy(n => n.NpcMonsterVNum);

            foreach (var monsterSkillGrouping in skills)
            {
                _monsterSkills[monsterSkillGrouping.Key] = monsterSkillGrouping.Select(n => new NpcMonsterSkill(n)).ToList();
            }

            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("MONSTERSKILLS_LOADED"), _monsterSkills.Sum(i => i.Count)));
        }

        private void LoadBattlePassItems()
        {
            var bpItems = DAOFactory.BattlePassItemDAO.LoadAll();
            BattlePassItems.AddRange(bpItems);
            Logger.Log.Info($"{BattlePassItems.Count} battlepass items loaded.");
        }

        private void LoadBattlePassPaliers()
        {
            var bpPaliers = DAOFactory.BattlePassPalierDAO.LoadAll();
            BattlePassPaliers.AddRange(bpPaliers);
            Logger.Log.Info($"{BattlePassPaliers.Count} battlepass paliers loaded.");
        }

        private void LoadBattlePassQuests()
        {
            var bpQuests = DAOFactory.BattlePassQuestDAO.LoadAll();
            BattlePassQuests.AddRange(bpQuests);
            Logger.Log.Info($"{BattlePassQuests.Count} battlepass quests loaded.");
        }

        private void LoadBCards()
        {
            var bcards = DAOFactory.BCardDAO.LoadAll();
            ...
            Logger.Log.Info($"{_bcards.Count} BCards loaded.");
        }

        private void LoadNpcMonsters()
        {
            // initialize npcmonsters
            var monsters = DAOFactory.NpcMonsterDAO.LoadAll();

            foreach (var npcMonster in monsters)
            {
                NpcMonster npcMonsterObj = new NpcMonster(npcMonster);
                npcMonsterObj.Initialize();
                npcMonsterObj.BCards = new List<BCard>();
                _bcards.Where(s => s.NpcMonsterVNum.Equals(npcMonster.OriginalNpcMonsterVNum > 0 ? npcMonster.OriginalNpcMonsterVNum : npcMonster.NpcMonsterVNum)).ToList().ForEach(s => npcMonsterObj.BCards.Add(new BCard(s)));
        ...
        public void Initialize()
        {
            ...
            ChatLogs = new ConcurrentBag<ChatLogDTO>();
            TitleWearConditions = new ConcurrentBag<TitleWearConditionDTO>();
            LevelUpRewards = new ConcurrentBag<LevelUpRewardDTO>();
            LoadBossEntities();
            LastFcRaid = FactionType.None;
            ...
            try
            {
                ...
                {
                    Logger.Log.Error(Language.Instance.GetMessageFromKey("NO_MAP"), null);
                }

                Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("MAPMONSTERS_LOADED"), monstercount));
                StartedEvents = new ThreadSafeGenericList<EventType>();
                ...
                Logger.Log.Info($"Scripted instances loaded");
                LoadBannedCharacters();
                Logger.Log.Info($"Banned characters loaded");

                var titleConditions = DAOFactory.TitleWearConditionDao.LoadAll();
                titleConditions.ToList().ForEach(x => TitleWearConditions.Add(x));
                Logger.Log.Info($"{titleConditions.Count()} Title Conditions loaded");

                var levelupRewards = DAOFactory.LevelUpRewardsDao.LoadAll();
                levelupRewards.ToList().ForEach(x => LevelUpRewards.Add(x));
                Logger.Log.Info($"{levelupRewards.Count()} Level Up Rewards loaded");

                LoadBattlePassItems();
                LoadBattlePassPaliers();
        ...
        public void LoadMimicRewards()
        {
            MimicItems = new List<MimicRotationDTO>();
            MimicItems.AddRange(DAOFactory.MimicRotationDAO.LoadByRotationType(MimicRotationType.Any));
            MimicItems.AddRange(DAOFactory.MimicRotationDAO.LoadByRotationType(Configuration.MimicRotationType));
            Logger.Log.Info($"{MimicItems.Count} Mimic items loaded");
        }
        ...
        public void JoinMiniland(ClientSession session, ClientSession minilandOwner)
        {
            ...
            {
                foreach (Mate mate in session.Character.Mates.GetAllItems())
                {
            ...
            if (session.Character.Miniland.MapInstanceId != minilandOwner.Character.Miniland.MapInstanceId)
            {
                ...
                minilandOwner.Character.GeneralLogs.Add(new GeneralLogDTO { AccountId = session.Account.AccountId, CharacterId = session.Character.CharacterId, IpAddress = session.CleanIpAddress, LogData = "Miniland", LogType = "World", Timestamp = DateTime.Now });
        ...
        public void LoadScriptedInstances()
        {
            ...
            SkyTowers = new ConcurrentBag<ScriptedInstance>();
        ...
        public void SaveAll()
        {
            ...
            DAOFactory.CharacterTimespaceLogDAO.InsertOrUpdateFromList(TimespaceLogs);
        }

        public async Task ShutdownTaskAsync(int time = 5)
        {
            Logger.Log.Warn($"Time Before Shutdown: {time}");
        ...
        private static void OnShutdown(object sender, EventArgs e)
        {
            ...
            {
                Instance.TaskShutdown = Instance.ShutdownTaskAsync((short)sender);
        ...
        {
            foreach (MapInstance map in GetAllMapInstances().Where(s => s.Map.MapTypes.Any(m => m.MapTypeId == (short)MapTypeEnum.Act4) && s.Npcs.Count(o => o.NpcVNum == 2004 || o.NpcVNum == 2020 && o.IsOut) < s.Npcs.Count(n => n.NpcVNum == 2004 || n.NpcVNum == 2020)))
            {
                foreach (MapNpc i in map.Npcs.Where(s => s.IsOut && s.NpcVNum == 2004 || s.NpcVNum == 2020))
                {
        ...
        private void Act4Process()
        {
            ...
            void SummonMukraju(MapInstance instance, byte faction)
            {
                ...
                {
                    MonsterVNum = 556,
                    MapY = (short)(faction == 1 ? 92 : 95),
                    MapX = (short)(faction == 1 ? 114 : 20),
                ...
                InterChannelShout($"The {(faction == 1 ? "Angels" : "Demons")} summoned Lord Mukraju");
                ...
                Observable.Timer(TimeSpan.FromSeconds(faction == 1 ? Act4AngelStat.TotalTime : Act4DemonStat.TotalTime)).SafeSubscribe(s =>
                {
                    if (instance.Monsters.ToList().Any(m => m.MonsterVNum == monster.MonsterVNum))
                    {
                        ...
                        Sessions.ToList().ForEach(s => s.SendPacket(s.Character.GenerateFc()));
                    }
                });
            }

            int CreateRaid(FactionType faction)
            {
                ...
                InterChannelShout($"The {(faction == FactionType.Angel ? "Angels" : "Demons")} opened the {raidName} raid.");
                return rng;
            }
            ...
            if (DateTime.Now >= LastFCSent.AddSeconds(10))
            {
                Sessions.ToList().ForEach(s => s.SendPacket(s.Character.GenerateFc()));
                LastFCSent = DateTime.Now;
            }
        ...
        public void InterChannelShout(string message, long? destinationCharacterId = null, long sourceCharacterId = 0, MessageType type = MessageType.Shout)
        {
        ...
        {
            Logger.Log.Debug("Flushing logs");
            var result = DAOFactory.ChatLogsDAO.InsertOrUpdate(ChatLogs);

            if (result == SaveResult.Error)
            {
                Logger.Log.Error("Cannot clear logs ! Error during insertion !", null);
                ChatLogs.Clear();
                return;
            }

            Logger.Log.Debug($"Logs flushed");
        }

        private void RunAct4Percent()
        {
            if (ChannelId != 51)
            {
                return;
            ...
            if (Act4AngelStat.Mode != 0 || Act4DemonStat.Mode != 0)
            {
                return;
            }

            InterChannelShout($"Glacernon stats. Angels: {Act4AngelStat.Percentage / 100}% | Demons: {Act4DemonStat.Percentage / 100}%");
        }
        ...
        private void LaunchEvents()
        {
            ...
            Observable.Interval(TimeSpan.FromSeconds(5)).SafeSubscribe(x => Act6Process());
            Observable.Interval(TimeSpan.FromSeconds(1)).SafeSubscribe(x => Act4Process());
            Observable.Interval(TimeSpan.FromSeconds(2)).SafeSubscribe(x => GroupProcess());
            Observable.Interval(TimeSpan.FromMinutes(1)).SafeSubscribe(x => Act4FlowerProcess());
            Observable.Interval(TimeSpan.FromHours(1)).SafeSubscribe(x =>
            {
                switch (_currentMessagesIndex)
                {
                    case 0:
                        RunAct4Percent();
                    ...
                    case 1:
                        InterChannelShout(MessagesLoop[_currentMessagesIndex - 1]);
                        _currentMessagesIndex++;
                        break;

                    case 2:
                        InterChannelShout(MessagesLoop[_currentMessagesIndex - 1]);
                        _currentMessagesIndex = 0;
                        break;
                }
            });
            Observable.Interval(TimeSpan.FromMinutes(15)).SafeSubscribe(x => LargeHeapCompactor.CompactHeap());
            EventHelper.GenerateEvent(EventType.TALENTARENA);

            Observable.Interval(TimeSpan.FromMinutes(1)).SafeSubscribe(x => CommunicationServiceClient.Instance.CleanupOutdatedSession());

            Observable.Interval(TimeSpan.FromSeconds(5)).SafeSubscribe(x => MaintenanceProcess());
            ...
            Parallel.ForEach(Schedules, schedule => Observable.Timer(TimeSpan.FromSeconds(EventHelper.GetMilisecondsBeforeTime(schedule.Time).TotalSeconds), TimeSpan.FromDays(1)).SafeSubscribe(e =>
            {
                if (schedule.DayOfWeek?.Length == 0 || schedule.DayOfWeek == DateTime.Now.DayOfWeek.ToString())
                {
                    if (schedule.Event == EventType.LOD)
                    {
                        InterChannelShout($"The Land of Death has now begun in Channel {schedule.ChannelId}");
                        EventHelper.GenerateEvent(schedule.Event, schedule.ChannelId, schedule.LvlBracket);
                    }
                    else
                    {
            ...
            Observable.Interval(TimeSpan.FromSeconds(1)).SafeSubscribe(x => RemoveItemProcess());
            Observable.Interval(TimeSpan.FromMilliseconds(400)).SafeSubscribe(x =>
            {
                Parallel.ForEach(_mapinstances, map =>
                {
        ...
        {
            BannedAccounts = new List<long>();
            DAOFactory.PenaltyLogDAO.LoadExpiredBans().ToList().ForEach(x =>
            {
                BannedAccounts.Add(x.AccountId);
            });
        }
        ...
        private void LoadTimespaceLogs()
        {
            TimespaceLogs = DAOFactory.CharacterTimespaceLogDAO.LoadAll();
            Logger.Log.Info($"{TimespaceLogs.Count} timespaces logs loaded.");
        }

        private void LoadFamilies()
        {
            OrderablePartitioner<FamilyDTO> familyPartitioner = Partitioner.Create(DAOFactory.FamilyDAO.LoadAll(), EnumerablePartitionerOptions.NoBuffering);
            FamilyList = new ThreadSafeSortedList<long, Family>();

            Parallel.ForEach(familyPartitioner, new ParallelOptions() { MaxDegreeOfParallelism = 8 }, familyDto =>
            {
            ...
            Logger.Log.Info($"{FamilyList.Count} families loaded.");
        }

        private void MaintenanceProcess()
        {
            List<ClientSession> sessions = Sessions.Where(c => c.HasSelectedCharacter).ToList();
            ...
            if (maintenanceLog != null)
            {
                if (maintenanceLog.DateStart <= DateTime.Now)
                {
                    ...
                    sessions.Where(s => s.Account.Authority < AuthorityType.TGS).ToList().ForEach(session => session.Disconnect());
                }
        ...
        private void OnFamilyRefresh(object sender, EventArgs e)
        {
            ...
            lock (FamilyList)
            {
                if (famdto != null)
                {
                    ...
                    foreach (var session in Sessions.Where(s => newFam.FamilyCharacters.Any(m => m.CharacterId == s.Character.CharacterId)))
                    {
                ...
                else if (fam != null)
                {
                    ...
                    foreach (ClientSession sess in Sessions.Where(s => fam.FamilyCharacters.Any(f => f.CharacterId.Equals(s.Character.CharacterId))))
                    {
                        ...
                        sess.Character.LastFamilyLeave = DateTime.Now.Ticks;
        ...
        private void OnMailSent(object sender, EventArgs e)
        {
            ...
            if (session != null)
            {
                ...
                {
                    session.Character.MailList.TryAdd((session.Character.MailList.Count > 0 ? session.Character.MailList.OrderBy(s => s.Key).Last().Key : 0) + 1, mail);
            ...
            if (sender != null)
            {
                ...
                switch (message.Type)
                {
                    ...
                    case MessageType.Speaker:
                        var charName = DAOFactory.CharacterDAO.LoadById(message.SourceCharacterId).Name;
                        var channelId = CommunicationServiceClient.Instance.GetChannelIdByWorldId(message.SourceWorldId);
                        foreach (var s in Instance.Sessions)
                        {
                            var msg = message.SourceWorldId == Instance.WorldId ?
                                $"<Speaker> [{charName ?? "Name"}]:" :
                                $"<Speaker CH-{channelId}> [{charName ?? "Name"}]:";

                            var output = message.IsItemSpeaker ?
                                $"sayitem 1 {message.SourceCharacterId} 17 {msg.Replace(' ', '|')}|{message.Message}" :
                                $"say 1 {message.SourceCharacterId} 13 {msg + message.Message}";

                            s.SendPacket(output);
                        };
                        break;

                    case MessageType.ReceivePacket:
                        if (!message.DestinationCharacterId.HasValue || string.IsNullOrEmpty(message?.Message))
                        {
                            break;
                        }

                        var currentSession = Sessions.FirstOrDefault(s => s.Character.CharacterId == message.DestinationCharacterId.Value);

                        currentSession?.ReceivePacket(message.Message, true);
                    ...
                    case MessageType.Staff:
                        ...
                        foreach (var session in Sessions.Where(s => s.HasSelectedCharacter && s.Character.Authority >= AuthorityType.TGS))
                        {
                            session.SendPacket(session.Character.GenerateSay(message.Message, 12));
                        }
                        break;
                    case MessageType.KickChannel:
                    {
                        if (!int.TryParse(message.Message, out var ch))
                        {
                            break;
                        }
                        ...
                        Parallel.ForEach(Sessions, session => session.Disconnect());
                    }
                        break;
                }
            }
        ...
        private void OnStaticBonusRefresh(object sender, EventArgs e)
        {
            ...
            if (sess != null)
            {
                sess.Character.StaticBonusList = new ThreadSafeGenericLockedList<StaticBonusDTO>(DAOFactory.StaticBonusDAO.LoadByCharacterId(characterId).ToList());
            }
        }
        ...
        public void IncreaseFcPercentage(int value, FactionType faction, bool rate = true, bool reset = false)
        {
            if (ChannelId != 51)
            {
                return;
            ...
            {
                case FactionType.Angel:
                    Act4AngelStat.Percentage = reset ? 0 : Act4AngelStat.Percentage;
                    Act4AngelStat.Percentage += value * (rate ? Configuration.RateAct4Xp : 1); 
                    current = Act4AngelStat.Percentage;
                    break;
                case FactionType.Demon:
                    Act4DemonStat.Percentage = reset ? 0 : Act4DemonStat.Percentage;
                    Act4DemonStat.Percentage += value * (rate ? Configuration.RateAct4Xp : 1);
                    current = Act4DemonStat.Percentage;
                    break;
            }

            FrozenCrownClient.Instance.SetPercent(new FcPercentModel
            {
                Faction = faction,
                Percentage = current
            });
