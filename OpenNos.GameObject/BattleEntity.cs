...
namespace OpenNos.GameObject
{
    public class BattleEntity
    {
        ...
        public List<ShellEffectDTO> RuneWeaponEffects { get; }
        ...
        public bool IsEggPet => Mate != null && MateHelper.Instance.EggTypeMates.Any(s => s.Equals(Mate.NpcMonsterVNum));
            ...
            bool forced = false)
        {
            if (indicator.Card != null)
            {
                ...
                if (Character != null && (indicator.Card.BuffType == BuffType.Bad && Character.HasGodMode || Character.InvisibleGm) || Mate != null && (indicator.Card.BuffType == BuffType.Bad && Mate.Owner.HasGodMode || Mate.Owner.InvisibleGm))
                {
                    return;
                }
                ...
                if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.LotusSkills && s.SubType.Equals((byte)AdditionalTypes.LotusSkills.ChangeMoonSkills)))
                {
                    RemoveBuff(697);
                }

                if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.LotusSkills && s.SubType.Equals((byte)AdditionalTypes.LotusSkills.ChangeLotusSkills)))
                {
                    RemoveBuff(690);
                }
                    ...
                    case EntityType.Player:
                        {
                            ...
                            // User in SafeZone
                            if (MapInstance.MapInstanceId == ServerManager.Instance.ArenaInstance.MapInstanceId && MapInstance.Map.Tiles[Character.PositionX, Character.PositionY].Value != 0 && (MapInstance.Map.Tiles[Character.PositionX, Character.PositionY].Value != 16 || Character.PositionY == 35))
                            {
                                return false;
                            }

                            if (MapInstance.MapInstanceId == ServerManager.Instance.FamilyArenaInstance.MapInstanceId && (MapInstance.Map.Tiles[Character.PositionX, Character.PositionY].Value != 0))
                            {
                                return false;
                            }
                                ...
                                case EntityType.Player:
                                    {
                                        ...
                                        var king = Character.Session.CurrentMapInstance.Sessions.FirstOrDefault(s => s.Character.kingOfTheHill.IsKing && s.Character.kingOfTheHill.MapInstanceId == Character.Session.CurrentMapInstance.MapInstanceId);
                                        var queen = Character.Session.CurrentMapInstance.Sessions.FirstOrDefault(s => s.Character.kingOfTheHill.IsQueen && s.Character.kingOfTheHill.MapInstanceId == Character.Session.CurrentMapInstance.MapInstanceId);

                                        if (king != null && queen != null)
                                        {
                                            if (MapInstance.IsPVP && MapInstance.MapInstanceType == MapInstanceType.KingOfTheHill && (receiver.Character.kingOfTheHill.TeamType == Character.kingOfTheHill.TeamType || ((Character.Name == king.Character.Name || Character.Name == queen.Character.Name) && (receiver.Character.Name == king.Character.Name || receiver.Character.Name == queen.Character.Name))))
                                            {
                                                return false;
                                            }
                                        }

                                        if (MapInstance.IsPVP && MapInstance.MapInstanceType == MapInstanceType.KingOfTheHill && receiver.Character.kingOfTheHill.TeamType != Character.kingOfTheHill.TeamType)
                                        {
                                            return true;
                                        }
                                        else if (MapInstance.IsPVP && MapInstance.MapInstanceType == MapInstanceType.TwoVersusTwo && !receiver.Character.TwoVersusTwoBattle.IsDead)
                                        {
                                            if (Character.Group != null)
                                            {
                                                if (receiver.Character.Group.IsMemberOfGroup(Character.Session))
                                                {
                                                    return false;
                                                }
                                            }
                                        ...
                                        else if (MapInstance.IsPVP && MapInstance.MapInstanceType == MapInstanceType.TalentArenaMapInstance && !receiver.Character.TalentArenaBattle.IsDead)
                                        {
                                            return true;
                                        }
                                              ...
                                              || HasBuff(CardType.SpecialEffects, (byte)AdditionalTypes.SpecialEffects.AbleToFightPVP) && receiver.HasBuff(CardType.SpecialEffects, (byte)AdditionalTypes.SpecialEffects.AbleToFightPVP))
                                        {
                                            ...
                                            if (MapInstance.MapInstanceType != MapInstanceType.TalentArenaMapInstance && (Character.Group == null || !Character.Group.IsMemberOfGroup(receiver.Character.CharacterId)))
                                            {
                                                return true;
                                            }
                                        }
        ...
        {
            if (Buffs.FirstOrDefault(s => s.Card.BCards.Any(b => b.Type.Equals((byte)CardType.DamageConvertingSkill) && b.SubType.Equals((byte)AdditionalTypes.DamageConvertingSkill.TransferInflictedDamage)))?.Sender is BattleEntity SacrificeSender)
            {
        ...
        {
            foreach (var disposable in BCardDisposables.GetAllItems())
            {
                disposable?.Dispose();
            }
            BCardDisposables.ClearAll();

            foreach (var bf in Buffs.GetAllItems())
            {
                bf.StaticVisualEffect?.Dispose();
            }

            foreach (var bf in BuffObservables.GetAllItems())
            {
                bf?.Dispose();
            }
            BuffObservables.ClearAll();
            Buffs.ClearAll();
        }
        ...
        public string GenerateDm(int dmg) => $"dm {(short)UserType} {MapEntityId} {dmg} 1";
        ...
        public int[] GetBuff(CardType type, byte subtype, int secondData = -1)
        {
            ...
            foreach (BCard entry in BCards.Where(s => s?.Type.Equals((byte)type) == true && s.SubType.Equals((byte)(subtype)) && (secondData == -1 || s.SecondData == secondData)))
            {
            ...
            lock (Buffs)
            {
                foreach (Buff buff in Buffs.GetAllItems())
                {
                    ...
                    foreach (BCard entry in buff.Card.BCards
                        .Where(s => s.Type.Equals((byte)type) && s.SubType.Equals((byte)(subtype)) && (secondData == -1 || s.SecondData == secondData) && (s.CastType != 1 || (s.CastType == 1 && buff.Start.AddMilliseconds(buff.Card.Delay * 100) < DateTime.Now))))
                    {
                        if (entry.IsLevelScaled)
                        {
                            if (entry.IsLevelDivided)
            ...
            if (Character != null && Character.Skills != null)
            {
                ...
                foreach (BCard entry in PassiveSkillsBCards.Where(s => s?.Type.Equals((byte)type) == true && s.SubType.Equals((byte)(subtype)) && (secondData == -1 || s.SecondData == secondData)))
                {
                    if (entry.IsLevelScaled)
                    {
                        if (entry.IsLevelDivided)
            ...
            if (Character != null && Character.RuneEffectMain != null)
            {
            ...
            if (Character != null && Character.EffectFromTitle != null)
            {
                foreach (BCard entry in Character.EffectFromTitle.Where(s => s?.Type.Equals((byte)type) == true && s.SubType.Equals((byte)(subtype)) && (secondData == -1 || s.SecondData == secondData)))
                {
                    if (entry.IsLevelScaled)
                    {
                        if (entry.IsLevelDivided)
        ...
        public int GetDamage(int damage, BattleEntity damager, bool dontKill = false, bool fromDebuff = false)
        {
            ...
            {
                int[] percentDefense = GetBuff(CardType.RecoveryAndDamagePercent, (byte)AdditionalTypes.RecoveryAndDamagePercent.DecreaseSelfHP);
        ...
        public bool HasBuff(CardType type, byte subtype, bool castTypeNotZero = false)
        {
            ...
            {
                Logger.Log.Error("Error on HasBuff(CardType type, byte subtype, bool castTypeNotZero = false) method", ex);
                return false;
            }
        }
        ...
        public double HPLoad()
        {
            ...
            if (Character != null)
            {
                ...
                multiplicator += GetBuff(CardType.IncreaseRes, (byte)AdditionalTypes.IncreaseRes.IncreaseHpPercent)[0] / 100D;
                multiplicator += GetBuff(CardType.GetInDodge, (byte)AdditionalTypes.GetInDodge.IncreaseHpInPercent)[0] / 100D;
                multiplicator += GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPPercentIncreased)[0] / 100D;

                MaxHp = (int)((CharacterHelper.HPData[(byte)Character.Class, Level - 1] + hp + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPIncreased)[0] + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPMPIncreased)[0]) * multiplicator);
            }
            else
            {
                MaxHp = HpMax;
        ...
        public int HpPercent() => (int)(Hp / HpMax * 100D);
        ...
        {
            return new int[] { 921, 1385, 1428, 1499, 1519 }.Contains(vnum);
        }

        public bool IsArenapost(int vnum)
        {
            // TODO: Put real arenapost vnum there
            return vnum == 920;
        }
        ...
        public int MaxTargetedByMonstersCount(bool teamCheck)
        {
            if (MapInstance?.MapInstanceType == MapInstanceType.BaseMapInstance)
            {
                if (teamCheck)
                {
                    ...
                    {
                        case EntityType.Player:
                            return (1 + Character.Mates.ToList().Count(m => m.IsTeamMember || m.IsTemporalMate)) * 50;

                        case EntityType.Mate:
                            return (1 + Mate.Owner.Mates.ToList().Count(m => m.IsTeamMember || m.IsTemporalMate)) * 5;
                    }
                }
                else
                {
        ...
        public double MPLoad()
        {
            ...
            if (Character != null)
            {
                ...
                multiplicator += GetBuff(CardType.GetInDodge, (byte)AdditionalTypes.GetInDodge.IncreaseMpInPercent)[0] / 100D;
                multiplicator += GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumMPPercentIncreased)[0] / 100D;

                MaxMp = (int)((CharacterHelper.MPData[(byte)Character.Class, Level - 1] + mp + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumMPIncreased)[0] + GetBuff(CardType.MaxHPMP, (byte)AdditionalTypes.MaxHPMP.MaximumHPMPIncreased)[0]) * multiplicator);
            }
            else
            {
                MaxMp = MpMax;
        ...
        public void RemoveBuff(short id, bool removePermaBuff = false)
        {
            ...
            if (indicator?.Card != null)
            {
                lock (indicator)
                {
                    ...
                    if (indicator.Card.BCards.Any(s
                        => s.Type == (byte)CardType.SpecialAttack && s.SubType == (byte)AdditionalTypes.SpecialAttack.NoAttack
                        || s.Type == (byte)CardType.Move && s.SubType == (byte)AdditionalTypes.Move.MovementImpossible
                        || s.Type == (byte)CardType.FrozenDebuff && s.SubType == (byte)AdditionalTypes.FrozenDebuff.EternalIce
                        ))
                    {
                        if (Character != null)
                        {
                    ...
                    if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.SpecialEffects && s.SubType == (byte)AdditionalTypes.SpecialEffects.ShadowAppears)
                    ...
                    if (Character != null)
                    {
                        ...
                        if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.SpecialActions && s.SubType.Equals((byte)AdditionalTypes.SpecialActions.Hide))
                         || indicator.Card.BCards.Any(s => s.Type == (byte)CardType.FalconSkill && s.SubType.Equals((byte)AdditionalTypes.FalconSkill.Hide))
                         || indicator.Card.BCards.Any(s => s.Type == (byte)CardType.FalconSkill && s.SubType.Equals((byte)AdditionalTypes.FalconSkill.Ambush)))
                        {
                        ...
                        if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.FearSkill && s.SubType.Equals((byte)AdditionalTypes.FearSkill.MoveAgainstWill)))
                        {
                            Character.Session?.SendPacket($"rv_m {MapEntityId} 1 0");
                        }
                        if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.FearSkill && s.SubType.Equals((byte)AdditionalTypes.FearSkill.AttackRangedIncreased)))
                        {
                            if (!Buffs.Any(s => s.Card.BCards.Any(b => b.Type == (byte)CardType.FearSkill && b.SubType.Equals((byte)AdditionalTypes.FearSkill.AttackRangedIncreased))))
                            {
                        ...
                        if (indicator.Card.BCards.Any(s => s.Type == (byte)CardType.DarkCloneSummon
                            && s.SubType == (byte)AdditionalTypes.DarkCloneSummon.ConvertDamageToHPChance))
                        {
                        ...
                        switch (indicator.Card.CardId)
                        {
                            ...
                            case 340:
                                if (MapInstance?.Map?.MapTypes != null && MapInstance.Map.MapTypes.Any(s => s.MapTypeId == (short)MapTypeEnum.Act52))
                                {
                            ...
                            case 620:
                                {
                                    if (Character.Session != null && Character.SavedLocation != null && indicator.Sender?.Character?.CharacterId == Character.CharacterId)
                                    {
                                        ...
                                        if (skill != null)
                                        {
                                            ...
                                            Observable.Timer(characterSkill.LastUse).SafeSubscribe(s => Character.Session.SendPacket(StaticPacketHelper.SkillReset(skill.CastId)));
                                        }
                                    }
                                }
                                break;
