...
{
    internal static class TalentArenaEvent
    {
        ...
        internal static void Matchmaking(ClientSession session)
        {
            ...
            {
                var opponent = ServerManager.Instance.ArenaMembers.FirstOrDefault(s => s.Session.Character.CharacterId != session.Character.CharacterId && s.ArenaType == EventType.TALENTARENA && CompetitiveRank.GetRankFromRp(s.Session.Character.RP).CanCompeteAgainst(getSessionRankFromRP));
                action(opponent);
            });
            ...
            Observable.Timer(TimeSpan.FromMinutes(2)).SafeSubscribe(x =>
            {
                ...
                session.Character.ArenaDisposable = observable.SafeSubscribe(x =>
                {
                    ...
                    var opponent = ServerManager.Instance.ArenaMembers.FirstOrDefault(s => s.Session.Character.CharacterId != session.Character.CharacterId && s.ArenaType == EventType.TALENTARENA && s.Session.Character.HeroLevel == closestSession.Session.Character.HeroLevel);
        ...
        private static void GenerateWin(ClientSession session)
        {
            ...
            session.Character.RP = CompetitiveRank.Win(session.Character.RP);
            ...
            var quest2 = ServerManager.Instance.BattlePassQuests.Find(s => s.MissionSubType == BattlePassMissionSubType.ParticipateAoTRanked);
            if (quest2 != null)
            {
                session.Character.IncreaseBattlePassQuestObjectives(quest2.Id, 1);
            }
        ...
        private static void GenerateLose(ClientSession session)
        {
            ...
            session.Character.RP = CompetitiveRank.Loose(session.Character.RP);

            session.SendPacket(UserInterfaceHelper.GenerateMsg("Opponent won the game, you will be teleported in 2 seconds", 0));

            var quest = ServerManager.Instance.BattlePassQuests.Find(s => s.MissionSubType == BattlePassMissionSubType.ParticipateAoTRanked);
            if (quest != null)
            {
                session.Character.IncreaseBattlePassQuestObjectives(quest.Id, 1);
            }
