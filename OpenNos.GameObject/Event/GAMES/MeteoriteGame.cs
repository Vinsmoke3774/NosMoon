    ...
    public static class MeteoriteGame
    {
        ...
        public static void GenerateMeteoriteGame()
        {
            ...
            if (map != null)
            {
                ...
                Observable.Timer(TimeSpan.FromSeconds(10)).SafeSubscribe(X => task.Run(map));
            }
        }
                    ...
                    Observable.Timer(TimeSpan.FromSeconds(3)).SafeSubscribe(observer =>
                    {
                        if (_map != null)
                        {
                            ...
                            foreach (Character character in _map.GetCharactersInRange(cell.X, cell.Y, 2))
                            {
                                ...
                                Observable.Timer(TimeSpan.FromMilliseconds(1000)).SafeSubscribe(o => ServerManager.Instance.AskRevive(character.CharacterId));
                            }
