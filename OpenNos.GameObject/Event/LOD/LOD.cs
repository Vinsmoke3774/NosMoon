            ...
            while (lodTime > 0)
            {
                ...
                if (lodTime == hornTime || (lodTime == hornTime - (hornRespawn * dhspawns)))
                {
                    foreach (Family fam in ServerManager.Instance.FamilyList.GetAllItems())
                    {
                        ...
                        {
                            EventHelper.Instance.RunEvent(new EventContainer(fam.LandOfDeath, EventActionType.CHANGEXPRATE, 8));
                            EventHelper.Instance.RunEvent(new EventContainer(fam.LandOfDeath, EventActionType.CHANGEDROPRATE, 8));
