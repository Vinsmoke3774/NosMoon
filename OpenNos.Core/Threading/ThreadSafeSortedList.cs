...
{
    /// <summary>
    /// This class is used to store key-value based items in a thread safe manner. It uses <see
    /// cref="SortedList"/> publicly.
    /// </summary>
    ...
    public class ThreadSafeSortedList<TK, TV> : IDisposable
    {
        ...
        public List<TV> GetAllItems()
        {
            ...
            {
                Logger.Logger.Log.Error(null, e);
            }
            return new List<TV>();
        }
        ...
        public List<TV> GetAndClearAllItems()
        {
            if (!_disposed)
            {
                ...
                try
                {
                    List<TV> list = new(_items.Values);
                    _items.Clear();
                    return list;
                }
                finally
        ...
        public IDictionary<TK, TV> ToDictionary()
        {
            if (!_disposed)
            {
                _lock.EnterReadLock();
                try
                {
                    return _items.ToDictionary(x => x.Key, y => y.Value);
                }
                finally
                {
                    _lock.ExitReadLock();
