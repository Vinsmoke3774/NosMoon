...
namespace OpenNos.Core.Networking.Communication.Scs.Communication.Protocols.BinarySerialization
{
    ...
    public class BinarySerializationProtocol : IScsWireProtocol, IDisposable
    {
        ...
        /// <returns>
        /// List of messages. Protocol can generate more than one message from a byte array. Also,
        /// if received bytes are not sufficient to build a message, the protocol may return an
        /// empty list (and save bytes to combine with next method call).
        /// </returns>
        ...
        private bool ReadSingleMessage(ICollection<IScsMessage> messages)
        {
            ...
            // If stream has less than 4 bytes, that means we can not even read length of the
            // message So, return false to wait more bytes from remore application.
