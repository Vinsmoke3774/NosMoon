...
{
    /// <summary>
    /// A random number generator based on the RNGCryptoServiceProvider. Adapted from the "Tales
    /// from the CryptoRandom" article in MSDN Magazine (September 2007) but with explicit guarantee
    /// to be thread safe. Note that this implementation also includes an optional (enabled by
    /// default) random buffer which provides a significant speed boost as it greatly reduces the
    /// amount of calls into unmanaged land.
    /// </summary>
    public class CryptoRandom : Random, IDisposable
    {
        ...
        /// disregard whatever value is passed as seed and it's only implemented in order to be
        /// fully backwards compatible with <see cref="System.Random"/>. Using this overload will
        /// enable the random buffer pool.
        /// </summary>
